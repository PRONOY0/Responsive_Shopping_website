...
const Women = ({ Products }) => {
  ...
  const [visibleProducts, setVisibleProducts] = useState(Products.filter(item => item.type === 'women').slice(i, j));

  function nextPageHandler() {
    const newI = i + totalItemsPerPage;
    const newJ = j + totalItemsPerPage;
    const itemsPerPage = Products.filter(item => item.type === 'women').slice(newI, newJ);
    setI(newI);
    setJ(newJ);
  ...
  function backHandler() {
    ...
    const previousBtnItemsPerPage = Products.filter(item => item.type === 'women').slice(oldI, oldJ);
    setI(oldI);
    setJ(oldJ);
  ...
  <div className="overflow-x-hidden scroll-smooth">
    ...
    <div>
    {
        ...
        Products.filter((item) =>
         singleClick.includes(item.category))
           ...
           .map((item) => <WomenCard item={item} key={item.id}/>)
        )


        :

        visibleProducts.map((item) => <WomenCard item={item} key={item.id} />)}
    ...
    <div
      ...
      >
        ...
        <div
          className={`${
            pageNo < 5 ? "opacity-100" : "opacity-0"
